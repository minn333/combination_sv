...
vcfdir = "...\\combination_sv\\step2_combi_intersect\\HG002.annotated.refine.replaced.filteredrange\\" + inputdir + "\\"

output_dir = "...\\combination_sv\\step2_combi_intersect\\HG002.annotated.refine.replaced.filteredrange\\" + inputdir + "\\" + outputdir + "\\"
...
with open(output_dir +sample + "." + mergefile + ".vcf","r") as merge_vcf:
    ...
    with open(output_dir +sample + "." + firstfile + ".vcf","w") as significantsv_first_f:
        with open(output_dir +sample + "." + allfile + ".vcf","w") as significantsv_all_f:
            ...
            for m in svs:
                if(len(m.neighbor_sv)>1) and (m.callerNumber()>1):  # caller > 1 

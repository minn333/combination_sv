...
truth_dir = "...\\combination_sv\\sv_man_package500\\svlen\\giab\\"

output_truth_dir = "...\\combination_sv\\sv_man_package500\\svlen\\giab\\truth\\"
...
with open(truth_dir + "HG002_SVs_Tier1_v06.refine.standardrange.vcf", "r") as truth_f:
    del_truth_vcf=[]
    ins_truth_vcf=[]
        ...
        if(line[0]!="#" and line[0:2]!="##"):
            ...
            for x in info_value:
                if "SVTYPE" in x:
                    ...
                    if svtype == "DEL":
                        del_truth_vcf.append(Variant("chr"+item[0], item[1], svtype))  ## 加"chr"
                        DEL_truth_count += 1
            for x in info_value:
                if "SVTYPE" in x:
                    svtype = x.split("=")[1].strip()
...
with open(output_truth_dir + "HG002.truthdel.vcf", "w") as del_truth_f:
    for m in del_truth_vcf:
        del_truth_f.write(str(m).strip() + "\n") 

with open(output_truth_dir + "HG002.truthins.vcf", "w") as ins_truth_f:
...
for input in os.listdir(test_dir):
    if input.lower()[:].find(".vcf") != -1:
        ...
        word = input.split('.')
        output = word[0] + "." + word[1] + "range." + word[2]
        caller = word[1] + "_" + word[-2]
...
for i in range(len(inputs)):
    ...
    with open(write_output + ".del." + callers[i] + ".vcf", "w") as del_test_f:
        for m in del_test_vcf:
            del_test_f.write(str(m).strip() + "\n") 
    
    with open(write_output + ".ins." + callers[i] + ".vcf", "w") as ins_test_f:
